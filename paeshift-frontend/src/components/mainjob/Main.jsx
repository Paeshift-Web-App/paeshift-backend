...
useEffect((job_id) => {
  Axios.get(`http://localhost:8000/jobs/jobs/${job_id}/shifts/`)
    .then((response) => {
      setJobs(response.data.jobs);
      console.log(response.data.jobs);
    })
...
const handleStartShift = (job_id) => {
  useEffect(() => {
    Axios.post(`http://localhost:8000/jobs/jobs/${job_id}/start-shift/`)
      .then((response) => {
        setJobs(response.data.jobs);
        console.log(response.data.jobs);
      })
...
const handleEndShift = (job_id) => {
  ...
  useEffect(() => {
    Axios.post(`http://localhost:8000/jobs/jobs/${job_id}/end-shift/`)
      .then((response) => {
        setJobs(response.data.jobs);
        console.log(response.data.jobs);
      })
...
const handleCancelJob = (job_id) => {
  ...
  useEffect(() => { 
    Axios.post(`http://localhost:8000/jobs/job/cancel/${job_id}/`)
      .then((response) => {
        setJobs(response.data.jobs);
        console.log(response.data.jobs);
      })
                  ...
                  </div>
                  {
                    item.status === 'upcoming' ?
                      profile.role === 'client' ?
                        ...
                        <div className="bottom">
                          <span>
                            <button className="cancel" onClick={()=>handleCancelJob(job_id)} >Cancel</button>
                          </span>
                          <span>
                            <Link to="../jobdetails" className="track-btn">Track Location</Link>
                          </span>
                        </div> :
                      ""
                  }
